...
static inline void print_allele_with_context(const bam_pileup1_t *p, int l_ref, const char *ref, int pos, int max_del, int is_vcf, int del_as_allele, int context, int svinfo)
{ // print the allele. The format depends on is_vcf.
	...
	if(svinfo){
		printf("SVTYPE=%s;", p->indel > 0? "INS" : (p->indel < 0 ? "DEL" : "SNP"));

		if (p->indel != 0){
			printf("SVLEN=%i;", p->indel);
		}
		...
		if (p->indel > 0){
			printf("TIG_REGION=%s:%d-%d;", p->b->data, p->qpos, p->qpos+p->indel );
			printf("CONTEXT=");

			for(i = p->qpos - context; i < p->qpos + p->indel + context ; i++){
				if(i < 0)
					continue;

				putchar(seq_nt16_str[bam_seqi(seq, i)]);
			}
		...
		if (p->indel < 0){
			printf("TIG_REGION=%s:%d-%d;", p->b->data, p->qpos, p->qpos+1 );
			printf("CONTEXT=");

			for(i = p->qpos - context; i < p->qpos  + context ; i++){
				if(i < 0)
					continue;
	...
	// for(i = p->qpos - context; i < p->qpos; i++){
	// 	if(i < 0)
	// 		continue;
	...
	// 	putchar(seq_nt16_str[bam_seqi(seq, p->qpos + abs(p->indel) + i)]);
	// }
}
...
static inline void print_allele(const bam_pileup1_t *p, int l_ref, const char *ref, int pos, int max_del, int is_vcf, int del_as_allele) {
	print_allele_with_context(p, l_ref, ref, pos, max_del, is_vcf, del_as_allele, 0, 0);
}
...
int main_pileup(int argc, char *argv[])
{
	...
	while ((n = getopt(argc, argv, "w:r:q:Q:l:f:dvcCS:Fs:D:V:uRMb:T:x:e")) >= 0) {
		...
		else if (n == 'w') context = atoi(optarg);
	...
	if (optind == argc) {
		...
		fprintf(stderr, "         -w         window size for context sequence [%i] \n", context);
		fprintf(stderr, "\n");
		return 1;
	}
	...
	if (is_vcf) {
		...
		puts("##INFO=<ID=SVTYPE,Number=1,Type=String,Description=\"SVTYPE\">");
		puts("##INFO=<ID=SVLEN,Number=1,Type=Integer,Description=\"SVLEN\">");
		puts("##INFO=<ID=TIG_REGION,Number=1,Type=String,Description=\"contig region\">");
		puts("##INFO=<ID=CONTEXT,Number=1,Type=String,Description=\"The context sequence surrounding the indel or SNP\">");
		...
		if (aux.tot_dp) {
			...
			if (is_fa) { // FASTA output
				...
				if (!is_vcf || a[0].hash>>63) { // print if there is no reference allele
					const bam_pileup1_t * p = &plp[a[0].pos>>32][(uint32_t)a[0].pos];
						//printf("%s", p->b->data);

					print_allele(p, l_ref, ref, pos - beg, aux.max_del, is_vcf, del_as_allele);
					if (aux.n_alleles > 1) putchar(',');
				}
				//printf("!");
				for (i = k = 1; i < aux.n_a; ++i)
					if (a[i].indel != a[i-1].indel || a[i].hash != a[i-1].hash) {
						const bam_pileup1_t * p = &plp[a[i].pos>>32][(uint32_t)a[i].pos];
						//printf("%s", p->b->data);
						print_allele(p, l_ref, ref, pos - beg, aux.max_del, is_vcf, del_as_allele);
				...
				if (is_vcf) {
					...
					if (!is_vcf || a[0].hash>>63) { // print if there is no reference allele
						...
						print_allele_with_context(p, l_ref, ref, pos - beg, aux.max_del, is_vcf, del_as_allele, context, 1);
						if (aux.n_alleles > 1) putchar(',');
					}
					//printf("!");
					for (i = k = 1; i < aux.n_a; ++i)
				...
				if (is_vcf) printf("\tGT:%s", show_2strand? "ADF:ADR" : "AD");
				// print counts
