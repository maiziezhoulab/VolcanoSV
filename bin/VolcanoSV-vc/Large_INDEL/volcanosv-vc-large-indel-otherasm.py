...
parser.add_argument('--input_path','-i', help = "whole genome scale haplotype-resolved fasta file that includes both haplotype. Should include 'hp1' or 'hp2' in contig name to differentiate haplotype.")
...
def align_contig(contig_path, reference_path, bam_path, n_thread, mem_per_thread,):
	
	# prefix = contig_path.split('/')[-1].split('.')[0]
	# other asm use asm20, volcanosv-asm use asm5
...
def split_contigs_by_chromosome(bam_file, contig_file, output_folder):
    """
    ...
    for chrom, contig_names in tqdm(chromosome_contigs.items(),desc ="write by chrom contigs"):
        if contig_names:
...
# align contig
logger.info(f"Aligning contig {input_path} to reference {reference}...")
align_contig(input_path, reference , contig_bam, n_thread, mem_per_thread,)

# split contig
logger.info("split assemblies by chromosome...")
split_contigs_by_chromosome(contig_bam, input_path, contig_by_chr_folder)
