...
def remove_duplicate(in_list, n_thread, ):
    ...
    print("%d fastq(s) have already been processed"% len(done_list))
...
def run_assembly_one_folder(hap_file, fastq_dir,  use_dir,assembler, 
    ...
    if hap_file is not None:
        ...
        pattern = os.path.join(fastq_dir, f"/*.fastq")
    ...
    if data_type!= 'Hifi':
        ...
        # update fq list
        found_files = [os.path.dirname(fq)+'_rmrd/'+os.path.basename(fq)   for fq in found_files ]

        output_dir = use_dir + "/Assembly_Output_" + os.path.basename(fastq_dir)+'_rmrd'
    else:
        output_dir = use_dir + "/Assembly_Output_" + os.path.basename(fastq_dir)
    # ----------- assembly
...
def run_assembly(hap_file, fastq_dirs, output_dir, assemblers, 
    ...
    for i in range(len(fastq_dirs)):
        ...
        run_assembly_one_folder(hap_file, fastq_dir,  output_dir, assembler, 
                 n_thread, asm_thread, clean, data_type, shasta_ont_config,
                  pacbio_subtype, prefix )
        fasta = os.path.dirname(fastq_dir) + "/Assembly_Output_" + os.path.basename(fastq_dir)+"/final_contigs/final_contigs.fa"
    ...
    cmd = f"cat {' '.join(fasta_list)} > {final_contig_dir}/final_contigs.fa"
    Popen(cmd, shell = True).wait()
...
if __name__ == "__main__":
    ...
    fastq_dirs = [  input_dir +  f"/chr{i+1}/Assembly/fastq_by_hap" for i in range(22)]
