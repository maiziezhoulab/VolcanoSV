...
task findBlocksByWindow {
    ...
    command <<<
        ...
        find tmp/ -name '*.error.bed' -print0 | xargs -0 cat | bedtools sort -i - | bedtools merge -i - > ~{suffix}/${PREFIX}.~{suffix}.error.bed
        find tmp/ -name '*.duplicated.bed' -print0 | xargs -0 cat | bedtools sort -i - | bedtools merge -i - > ~{suffix}/${PREFIX}.~{suffix}.duplicated.bed
        find tmp/ -name '*.haploid.bed' -print0 | xargs -0 cat | bedtools sort -i - | bedtools merge -i - > ~{suffix}/${PREFIX}.~{suffix}.haploid.bed
        find tmp/ -name '*.collapsed.bed' -print0 | xargs -0 cat | bedtools sort -i - | bedtools merge -i - > ~{suffix}/${PREFIX}.~{suffix}.collapsed.bed
