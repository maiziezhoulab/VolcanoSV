...
def plotProbTable(table, ax, title, zoomed = False, xlim_ratio=3):
    ...
    if zoomed:
        max_y = 0.1 + 2 * max(np.max(table["collapsed"] * fit), \
                              np.max(table["duplicated"] * fit), \
                              np.max(table["error"] * fit))
    else:
        max_y = 1.25 * np.max(fit)
    ax.set_xlim(0, max_x)
    ax.set_ylim(0, max_y)
...
def plotPairDist(table, pdf, suptitle, stats_file = None):
    ...
    plotSummaryTable(ax3, table, stats_file)
...
def plotSummaryTable(ax, table, stats_file):
    ...
    if stats_file is None:
        ...
        error_per = df[df['label'] =='Err']['perct'].values[0]
        duplicated_per = df[df['label'] =='Dup']['perct'].values[0]
        haploid_per = df[df['label'] =='Hap']['perct'].values[0]
        collapsed_per = df[df['label'] =='Col']['perct'].values[0]
        total_size = round( df[df['label'] =='Total']['size'].values[0] / 1e6, 3)
        data = np.array([ total_size, error_per, duplicated_per, haploid_per, collapsed_per])
        df = pd.DataFrame(data)
        ax.table(cellText=df.values, rowLabels=["Size (Mb)",
                                                "Error (%)",
                                                "Duplicated (%)",
...
def main():
    ...
    tableFiles = [join(tablesDir, f) for f in listdir(tablesDir) if f.endswith('.table')]
    ...
    for p in tableFiles:
        ...
        plotPairDist(table,  pdf, "{}\n{}\n".format(prefix, suffix),stats_file,)
    pdf.close()
